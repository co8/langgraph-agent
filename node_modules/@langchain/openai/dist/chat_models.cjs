    ...
    return messages.flatMap((lcMsg) => {
            ...
            if (toolMessage.additional_kwargs?.type === "computer_call_output") {
                const output = (() => {
                    ...
                    if (Array.isArray(toolMessage.content)) {
                        return toolMessage.content.find((i) => i.type === "computer_screenshot");
                    }
        ...
        if (role === "assistant") {
            ...
            if ((0, messages_1.isAIMessage)(lcMsg) && !!lcMsg.tool_calls?.length) {
                return lcMsg.tool_calls.map((toolCall) => ({
                    type: "function_call",
                    name: toolCall.name,
            ...
            if (lcMsg.additional_kwargs.tool_calls != null) {
                return lcMsg.additional_kwargs.tool_calls.map((toolCall) => ({
                    type: "function_call",
...
function _convertOpenAIResponsesMessageToBaseMessage(response) {
    ...
    for (const item of response.output) {
        if (item.type === "message") {
            ...
            content.push(...item.content.map((part) => {
